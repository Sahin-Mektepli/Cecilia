...
uint64_t Accuracy(Party *const proxy, uint64_t* prediction_shares, uint64_t test_shares, int size){
    if(proxy ->GetPRole()==helper){
...
uint64_t* Sigmoid(Party *const proxy, uint64_t* z,uint32_t size,int shift = FRACTIONAL_BITS){
    ...
    if(proxy ->GetPRole()==helper){
        cout << "sigmoid helper" << endl;

        Compare(proxy,nullptr,nullptr,size,shift);
        Compare(proxy,nullptr,nullptr,size,shift);
        cout <<"kıyaslar tamam" << endl;
        //Divide(proxy,nullptr,nullptr,size,shift);
