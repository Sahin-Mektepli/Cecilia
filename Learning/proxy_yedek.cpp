//
// 28.07.24 15.06'da aldığım yedektir.
// import hataları dosya yanlış klasörde olduğundan naşidir.
//
#include <cstdlib>
#include <iostream>
#include <chrono>
...
int main(int argc, char* argv[]) {
    ...
    vector<double> epsilons = {0}; //epsilonların kullanmaya alışkın olduğumuz bir konfigürasyonu
    for(auto epsilon: epsilons){
        ...
        for(int i=0; i<1; ++i){ //makalede 100 olarak verilen "run time"
            ...
            for(int j=0; j<columns; ++j){
                thetas[j] = proxy->CreateShare(.0);
            }
            //thetas[0] = proxy->CreateShare(.0);
            //thetas[1] = proxy->CreateShare(1.0);

            proxy->SendBytes(lgGradient, param, 2);
            uint64_t *new_thetas =  GradientDescent(proxy, X, y, thetas, rows, columns, 0.5, epsilon, 6); //X_train ve y_train ile
            ...
            cout << ConvertToDouble(Reconstruct(proxy, prediction[2])) << endl; //bu çıktıların anlamı azalmakta
